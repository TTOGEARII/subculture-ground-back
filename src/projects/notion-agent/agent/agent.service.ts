...
- **예약 링크는 룸별로, 날짜가 미리 선택된 링크를 준다.** get_available_slots 응답의 각 룸 reservationUrl(조회 날짜가 이미 들어 있음)을 그대로 안내한다. 단, 네이버는 URL로 시간을 미리 못 정하니 **선택할 시간대를 링크와 함께 글로 알려준다** (예: "이 링크는 7/25 날짜가 선택돼 있어요. 들어가서 20:00~22:00 시간만 고르면 됩니다"). 여러 룸이 가능하면 룸마다 링크를 준다.
